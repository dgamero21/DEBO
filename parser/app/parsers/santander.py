"""Banco Santander parser.

Parses Santander Río credit card statement PDFs.
Supports Visa, Mastercard, and Amex statements.
"""
...
class SantanderParser(BaseParser):
    """Parser for Banco Santander Río credit card statements."""
    ...
    def can_parse(self, text: str) -> bool:
        return bool(re.search(
            r"santander|banco\s+santander|santander\s+r[ií]o",
            text, re.IGNORECASE,
        ))
    ...
    def _extract_amounts(self, text: str) -> dict:
        ...
        m = re.search(
            r"(?:intereses|intereses\s*financiaci[oó]n)[:\s]*\$?\s*([\d.,\s]+)",
            text, re.IGNORECASE,
        )
        if m:
    ...
    def _extract_transactions(self, text: str) -> list[ParsedTransaction]:
        ...
        logger.info(f"Extracted {len(transactions)} transactions from Santander statement")
        return transactions
    ...
    def parse(self, text: str) -> ParsedStatement:
        """Parse Santander statement text into structured data."""
        logger.info("Parsing Santander Río statement...")
...
register_parser(SantanderParser())
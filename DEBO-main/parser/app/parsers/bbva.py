...
Parses BBVA Francés credit card statement PDFs.
...
class BbvaParser(BaseParser):
    """Parser for BBVA Francés credit card statements."""
    ...
    def can_parse(self, text: str) -> bool:
        return bool(re.search(
            r"bbva|bbva\s+franc[ée]s|banco\s+bbva|franc[ée]s",
            text, re.IGNORECASE,
        ))
    ...
    def _extract_transactions(self, text: str) -> list[ParsedTransaction]:
        ...
        logger.info(f"Extracted {len(transactions)} transactions from BBVA statement")
        return transactions
    ...
    def parse(self, text: str) -> ParsedStatement:
        """Parse BBVA statement text into structured data."""
        logger.info("Parsing BBVA Francés statement...")

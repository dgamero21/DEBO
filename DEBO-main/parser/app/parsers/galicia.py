"""Banco Galicia parser.

Parses Galicia credit card statement PDFs.
Supports Visa and Mastercard statements from Banco Galicia.
"""
...
class GaliciaParser(BaseParser):
    ...
    def can_parse(self, text: str) -> bool:
        return bool(re.search(r"galicia|banco\s+galicia", text, re.IGNORECASE))

    def _extract_card_info(self, text: str) -> tuple[str, str, Optional[str]]:
        """Extract card type, last four digits, and holder name."""
        card_type = "visa"
        card_last_four = "0000"
        holder_name = None

        # Card type detection
        if re.search(r"mastercard|master\s*card", text, re.IGNORECASE):
        ...
        m = re.search(r"(?:\*{4}|\.{4})\s*(?:\*{4}|\.{4})\s*(?:\*{4}|\.{4})\s*(\d{4})", text)
        if m:
            card_last_four = m.group(1)

        # Holder name: "Titular: NOMBRE APELLIDO" or similar
        m = re.search(r"(?:titular|holder|nombre)[:\s]*([A-ZÁÉÍÓÚÜÑ\s]{3,60})(?:\n|$)", text, re.IGNORECASE)
        if m:
    ...
    def _extract_dates(self, text: str) -> tuple[Optional[str], Optional[str], Optional[str]]:
        """Extract period_from, period_to, due_date."""
        period_from = None
        period_to = None
        due_date = None
    ...
    def _extract_amounts(self, text: str) -> dict:
        """Extract monetary values from the statement."""
        amounts = {
            "total_amount": None,
        ...
        m = re.search(
            r"(?:pago\s*total|total\s*a\s*pagar|saldo\s*total)[:\s]*\$?\s*([\d.,\s]+)",
            text, re.IGNORECASE,
        )
        if m:
        ...
        # Pago mínimo
        m = re.search(
            r"(?:pago\s*m[ií]nimo|pago\s*m[ií]n\.?)[:\s]*\$?\s*([\d.,\s]+)",
            text, re.IGNORECASE,
        )
        if m:
        ...
        # Pago recibido
        m = re.search(
            r"(?:pago\s*recibido|pago\s*acreditado)[:\s]*\$?\s*([\d.,\s]+)",
            text, re.IGNORECASE,
        )
        if m:
            amounts["payment_received"] = parse_amount(m.group(1))

        # Nuevos consumos
        m = re.search(
            r"(?:nuevos\s*consumos|consumos\s*del\s*per[ií]odo|compras)[:\s]*\$?\s*([\d.,\s]+)",
            text, re.IGNORECASE,
        )
        if m:
            amounts["new_charges"] = parse_amount(m.group(1))

        # Intereses
        m = re.search(
            r"(?:intereses|intereses\s*financiación|intereses\s*punitorios)[:\s]*\$?\s*([\d.,\s]+)",
            text, re.IGNORECASE,
        )
        if m:
            amounts["interest_charges"] = parse_amount(m.group(1))
    ...
    def _extract_transactions(self, text: str) -> list[ParsedTransaction]:
        ...
        if not tx_section:
            logger.warning("No transaction section found in Galicia statement")
            return transactions
        ...
        logger.info(f"Extracted {len(transactions)} transactions from Galicia statement")
        return transactions

    def _find_transaction_section(self, text: str) -> Optional[str]:
        """Find the transaction table portion of the statement."""
        markers = [
            r"consumos[:\s]*\n",
        ...
        section = text[start:]
        footer_markers = [
            r"\n\s*(?:resumen|totales|totales\s*del\s*per[ií]odo|totales\s*de\s*la\s*tarjeta)",
            r"\n\s*(?:totales?\s*\$)",
        ]
        end = len(section)
    ...
    def _parse_transaction_line(self, line: str) -> Optional[ParsedTransaction]:
        """Parse a single transaction line using shared utility."""
        return parse_transaction_line(line)

    def parse(self, text: str) -> ParsedStatement:
        """Parse Galicia statement text into structured data."""
        logger.info("Parsing Galicia statement...")
